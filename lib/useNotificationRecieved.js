...
export default function useNotificationRecieved(callback, useListener = false) {
    ...
    useEffect(() => {
        ...
        if (useListener) {
            subscription = Notifications.addNotificationResponseReceivedListener(response => {
                ...
                callback(response.notification.request.identifier);
            });
        }
        return () => {
            if (useListener && subscription) {
                subscription.remove();
            }
        };   
    });
